...
int main(int argc, char *argv[]){
    if(strcmp(argv[2],"megafile.txt") == 0 || strcmp(argv[2],"megafile-encoded.txt") == 0){
      errorf("Cant Process this file.");
      return 0;
    }
    if(argc != 3){
        errorf("Expected:\n");
    ...
    if(strcmp(argv[1], "--encode") == 0){ // Encode
        ...
        writeFile(fileName, newBuffer, newSize);
        ...
        strcpy(fileName, argv[2]);
        filename(fileName);
        strcat(fileName, "-decoded.txt");
        writeFile(fileName, newBuffer, newSize);
    } else {
        errorf("Error, expected: [--encode | --decode]\n");
        return -1;
    }
    return 0;
}